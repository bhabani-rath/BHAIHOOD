  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName("disable")
        .setDescription(
          "Disable the verification system and remove the verification channel."
        )
    )
    .addSubcommand((sub) =>
      sub
  ...
  async execute(interaction) {
    if (interaction.isCommand && interaction.isCommand()) {
      ...
      if (sub === "enable") {
        ...
        if (config?.verificationEnabled) {
          ...
          if (existingChannel) {
            return interaction.editReply({
              content:
                "⚠️ Verification system is already enabled for this server.",
            });
          }

          // Channel is missing — auto-cleanup
          console.log(
            `[ Verification ] Channel missing for guild ${guild.name}, reinitializing...`
          );
        ...
        for (const channel of guild.channels.cache.values()) {
          try {
            ...
            console.error(
              `Failed to update permissions for channel: ${channel.name}`,
              error
            );
        ...
        return interaction.editReply({
          content: "✅ Verification system enabled successfully!",
        });
      }
      ...
      if (sub === "disable") {
        if (!config?.verificationEnabled) {
          return interaction.editReply({
            content: "⚠️ Verification system is not currently enabled.",
          });
        }
        ...
        return interaction.editReply({
          content:
            "✅ Verification system disabled. Channel removed, roles preserved.",
        });
      }
      ...
      if (sub === "view") {
        if (!config?.verificationEnabled) {
          return interaction.editReply({
            content: "ℹ️ The verification system is currently disabled.",
          });
        }
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({
            text: "Use /setup-verification disable to reset settings.",
          })
          .setTimestamp();
